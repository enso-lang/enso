define(["enso"], (function (Enso) {
  var Dynamic;
  var DynamicUpdateProxy = MakeClass("DynamicUpdateProxy", EnsoProxyObject, [], (function () {
  }), (function (super$) {
    ...
    (this._get = (function (name) {
      ...
      if (var_V) { 
        ...
        if (((!System.test_type(name, self.Variable())) && name.start_with_P("_"))) { 
          return self.$.obj.send(name.to_sym()); 
               ...
               if (field.many()) { 
                 return self.$.obj._get(name); 
                      ...
                      if (System.test_type(val, self.Factory().MObject)) {
                        (val = val.dynamic_update());
                      }
                      self.$.fields._set(name, (var_V = self.Variable().new(S("", self.$.obj, ".", name, ""), val)));
                      self.$.obj.add_listener(self.name((function (val) {
