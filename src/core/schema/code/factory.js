...
MObject = MakeClass( {
  ...
  method_missing: function( block, sym ) {
    var self = this; 
    var args = compute_rest_arguments(arguments, 2 );
    var super$ = this.super$. method_missing ;
    if (sym._get(- "1") == "?") {
      self.schema_class().name() == sym.slice("0", sym.length() - "1");
    } else {
      super$.call_rest_args$(self, sym, args );
    }
  },
  ...
  __delete_obj: function( mobj ) {
    ...
    self.schema_class().fields().each(function(fld) {
      if (fld.traversal()) {
  ...
  dynamic_update: function( ) {
    ...
    self.$.dyn = self.$.dyn || DynamicUpdateProxy.new(self);
  },
  ...
  notify: function( name, val ) {
    ...
    if (self.$.listeners._get(name)) {
      self.$.listeners._get(name).each(function(blk) {
        return blk.call(val);
      });
    }
  },
  ...
  to_s: function( ) {
    ...
    k = ClassKey(self.schema_class());
    if (k) {
      S("<<", self.schema_class().name(), " ", self._id(), " '", self._get(k.name()), "'>>");
    } else {
  ...
  __init: function( fields, args ) {
    ...
    fields.each_with_index(function(fld, i) {
      if (i < args.length()) {
  ...
  __setter: function( name ) {
    ...
    self.define_singleton_method(function(arg) {
...
Single = MakeClass( Field, {
  ...
  set: function( value ) {
    ...
    self.check(value);
    self.$.value = value;
    self.$.owner.notify(self.$.field.name(), value);
  },
...
Prim = MakeClass( Single, {
  check: function( value ) {
    ...
    if (! self.$.field.optional() || value) {
      ...
      ;
      if (! ok) {
        self.raise(S("Invalid value for ", self.$.field.type().name(), ": ", value));
      }
    }
  },
  ...
  default: function( ) {
    ...
    if (! self.$.field.optional()) {
      if (self.$.field.type().name() == "str") {
        ...
        self.raise(S("Unknown primitive type: ", self.$.field.type().name()));
      }
    }
  }
});
    ...
    if (old != new_V) {
      ...
      if (self.$.inverse) {
        if (self.$.inverse.many()) {
          ...
          if (new_V) {
            new_V.__get(self.$.inverse.name()).__insert(self.$.owner);
          }
        } else {
          if (old) {
          ...
          if (new_V) {
            new_V.__get(self.$.inverse.name()).__set(self.$.owner);
          }
        }
      }
    }
...
Ref = MakeClass( Single, {
  ...
  _path: function( _ ) {
    var self = this; 
    var super$ = this.super$. _path ;
    self.$.owner._path().field(self.$.field.name());
  },
...
Many = MakeClass( Field, {
  ...
  set: function( ) {
    var self = this; 
    var super$ = this.super$. set ;
    self.raise(S("Cannot assign to many-valued field ", self.$.field.name()));
  },
...
Set = MakeClass( Many, {
  ...
  _path: function( mobj ) {
    var self = this; 
    var super$ = this.super$. _path ;
    self.$.owner._path().field(self.$.field.name()).key(mobj._get(self.$.key.name()));
  },
  ...
  __insert: function( mobj ) {
    ...
    self.$.value ._set( mobj._get(self.$.key.name()) , mobj );
  },
...
List = MakeClass( Many, {
  ...
  each_pair: function( block ) {
    var self = this; 
    var super$ = this.super$. each_pair ;
    self.__value().each_with_index(function(item, i) {
  ...
  _path: function( mobj ) {
    var self = this; 
    var super$ = this.super$. _path ;
    self.$.owner._path().field(self.$.field.name()).index(self.$.value.index(mobj));
  },
