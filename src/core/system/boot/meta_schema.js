   ...
   require ( "enso" )
     ...
     MObject = MakeClass( EnsoBaseObject, { _class_: { seq_no : 0 },
       ...
       schema_class : function() { var self=this; {
         ...
         self.define_singleton_method(function() { return res ; } ,
           "schema_class") ;
         return res ; } } ,
       _get : function(sym) { var self=this; {
         res = sym._get(- 1) == "?" ? self.schema_class().name() == sym.slice(
          0,
           sym.length() - 1) : self.$.data.has_key( str( sym, "=")) ? self.$.data._get(
           str( sym, "=")) : self.$.data.has_key( str( sym, "#")) ? Boot.make_field(
          self.$.data._get( str( sym, "#")),
           self.$.root,
           true) : self.$.data.has_key( sym.to_s()) ? Boot.make_field(
          self.$.data._get( sym.to_s()),
           self.$.root,
           false) : raise(
           str(
         ...
         self.define_singleton_method(function() { return res ; } , sym) ;
         return res ; } } ,
     ...
     Schema = MakeClass( MObject, { _class_: { },
       classes : function() { var self=this; { return BootManyField.new(
         self.types().select( function(t) { return t.Class() ; }),
         self.$.root,
         true) ; } } ,
       primitives : function() { var self=this; { return BootManyField.new(
         self.types().select( function(t) { return t.Primitive() ; }),
         self.$.root,
         true) ; } } })
     Class = MakeClass( MObject, { _class_: { },
       all_fields : function() { var self=this; { return BootManyField.new(
         self.supers().flat_map( function(s) { return s.all_fields() ; }) + defined_fields,
         self.$.root,
         true) ; } } ,
       fields : function() { var self=this; { return BootManyField.new(
     ...
     BootManyField = MakeClass( Array, { _class_: { },
       initialize : function(arr, root, keyed) { var self=this;
         ...
         self.$.root = root ;
         self.$.keyed = keyed ; } ,
       _get : function(key) { var self=this; { if (self.$.keyed) { return self.find( function(
        obj) { return obj.name() == key ; }) ; } else { return self.at( key) ; } } } ,
       has_key : function(key) { var self=this; { return self._get( key) ; } } ,
       join : function(block, other) { var self=this; { if (self.$.keyed) {
         ...
         a = Array( self) ;
         b = Array( other) ;
         return Range.new(0, [ a.length() , b.length()].max() - 1).each( function(
          i) { return block.call( a._get( i), b._get( i)) ; }) ; } } } ,
     ...
     make_object = function(data, root) { if ( data._get("class") == "Schema") { return makeProxy(
       Schema.new( data, root)) ; } else if ( data._get("class") == "Class") { return makeProxy(
       Class.new( data, root)) ; } else { return makeProxy(
       MObject.new( data, root)) ; } }
     make_field = function(data, root, keyed) { if ( data.is_a( Array)) { return make_many(
       data,
       root,
