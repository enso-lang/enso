...
class GLL3
  ...
  def scan_kind(kind)
    ...
    if tk then
      ...
      return if @keywords.include?(tk)
      ws, pos = skip_ws
      return pos, unescape(tk, kind), ws 
    end
  end
...
if __FILE__ == $0 then
  ...
  require 'core/schema/tools/print'
  require 'core/grammar/parse/build4'
  require 'core/grammar/parse/resolve'
  require 'core/grammar/parse/parse_'
  require 'core/grammar/parse/literals'
  require 'core/schema/code/factory'
  require 'core/expr/code/eval4'
  require 'core/semantics/factories/combinators'
  require 'core/semantics/factories/model-fold'
  require 'yaml'
  gg = Loader.load('grammar.grammar')
  gs = Loader.load('grammar.schema')
  f = FFold.new(Eval.new <
                Resolve.new <
                Build4.new < 
                Cyclic.new(literals: []) < 
                Literals.new < 
  ...
  puts f.literals.inspect

  gll = GLL3.new(f)

  file = 'core/expr/models/expr.grammar'
  src = File.read(file)
  x = gll.parse(src)
  Print.print(x)
