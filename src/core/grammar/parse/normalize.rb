...
module NormalizeGrammar
  ...
  def self.normalize(grammar)
    Normalizer.new(grammar.graph_identity).normalize(grammar, []).finalize
  end
  ...
  class Normalizer    
    ...
    def Sequence(this, added)
      rule = @fact.Rule
      rule.name = "Sequence_#{this.identity}"
    ...
    def Create(this, added)
      ...
      #end
      rule = @fact.Rule
      rule.name = "Create_#{this.name}_#{this.identity}"
    ...
    def Field(this, added)
      ...
      rule = @fact.Rule
      rule.name = "Field_#{this.name}_#{this.identity}"
    ...
    def Regular(this, added)
      ...
      if !this.many && this.optional then
        rule = @fact.Rule
        rule.name = "Optional_#{this.identity}"
        ...
        raise "Wrong sep: #{sep}" if !(sep.is_a?("Terminal") || sep.is_a?("Call"))
        rule = @fact.Rule
        rule.name = "IterPlusSep_#{this.identity}"
        ...
        raise "Wrong sep: #{sep}" if !(sep.is_a?("Terminal") || sep.is_a?("Call"))
        helper = @fact.Rule
        helper.name = "IterPlusSepHelper_#{this.identity}"
        ...
        added << helper
        rule = @fact.Rule
        rule.name = "IterStarSep_#{this.identity}"
        rule.arg = @fact.Alt(nil, nil, [@fact.Sequence(nil, nil, []), 
    ...
    def clone(arg)
      if arg.is_a?("Call") then
        @fact.Call(nil, nil, arg.rule)
      elsif arg.is_a?("Terminal") || arg.is_a?("Epsilon") then
