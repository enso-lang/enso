...
module SecureFactory
  ...
  module SecureMObjectMixin
    def [](name)
      ...
      auth1 = check_privileges("OpRead", self, name)
      if !auth1
    ...
    def []=(name, x)
      auth1 = check_privileges("OpUpdate", @self, name)
    ...
    def delete!
      auth = check_privileges("OpDelete", self)
  ...
  module SecureSingleMixin
    def get
      res = super
      auth2 = @owner.check_privileges("OpRead", res)
      !auth2 ? nil : res
    end
  end
  ...
  module SecureManyMixin
    ...
    def <<(mobj)
      auth = @owner.check_privileges("OpUpdate", @owner, @field.name)
      raise SecurityError, msg if !auth and !@fail_silent
      super if auth
    end

    def delete(mobj)
      auth2 = @owner.check_privileges("OpUpdate", @owner, @field.name)
      raise SecurityError, msg if !auth2 and !@fail_silent
      auth3 = @owner.check_privileges("OpDelete", mobj)
    ...
    def values
      super.select do |v|
        auth2 = @owner.check_privileges("OpRead", v)
        auth2
      end
    end
  ...
  def Make_Class(args=nil)
    res = super
    auth = args[:factory].check_privileges("OpCreate", res)
    if !auth
