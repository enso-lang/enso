...
\algblockdefx[Case]{Case}{EndCase}[1]{\textbf{case}\  #1\ \textbf{of}}[0]{\textbf{esac}}
\algcblockdefx[Case]{Case}{When}{EndCase}[1]{\textbf{when}\  #1:}[0]{\textbf{esac}}
\algcblockdefx[Case]{Case}{Else}{EndCase}[0]{\textbf{else}}[0]{\textbf{esac}}
...
\Function{instantiate}{$v: V$, $S: Schema$}
  ...
  \Function{build}{$v: V$, $o: S$, $f_S: Schema.classes[Field]$}
    ...
    \When{$\llbracket C\rrbracket(\overline{B})$} 
       \State $o'\gets \text{\textbf{new }}S.classes[C]$
       ...
       \State \Return{$o'$}
    \When{$P$}
       \State $fixups \gets fixups \cup \{\langle o, f_S, P\rangle\}$
         \Comment{Record a fixup for later patching}
       \State \Return{$\bot$}
    \Else
       \State \Return{}\Call{valueOf}{$v$, $f_S.type$}
